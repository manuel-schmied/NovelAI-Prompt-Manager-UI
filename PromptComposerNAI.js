...
(function() {
    ...
    function openPromptComposer() {
        ...
        for (let category in categories) {
            const label = $('<label></label>').text(category + ':').css({'margin-bottom': '5px', 'color': '#333'});
            const select = $('<select></select>')
                .css({'margin-bottom': '15px', 'padding': '5px', 'border-radius': '5px', 'width': '100%'})
                .append('<option value="">Select ' + category.toLowerCase() + '</option>');
            ...
            select.change(function() {
                selectedOptions[category] = $(this).val();
            });

            modalDiv.append(label).append(select);
        }
        ...
        const generateButton = $('<button>Generate Prompt</button>')
            ...
            .click(() => {
                const combinedPrompt = Object.values(selectedOptions).filter(val => val !== "").join(", ");
